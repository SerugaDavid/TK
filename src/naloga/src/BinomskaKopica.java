...
public class BinomskaKopica<Tip> implements Seznam<Tip> {
    ...
    private void fixStructure() {
        ...
        while (current != null) {
            if (prev != null && current.degree == prev.degree) {
                if (this.comparator.compare(current.value, prev.value) < 0) { //(current.value.compareTo(prev.value) < 0) {
                    max = prev;
                    min = current;
                }
                else {
    ...
    private ArrayList<Object> getMaxElement() {
        ...
        while (current.sibling != null) {
            if (this.comparator.compare(current.value, max) > 0) {// (current.sibling.value.compareTo(max) > 0) {
